...
用户可以在`template`目录下添加自己的模板文件，文件名可以自定义，但是需要保持`.txt`扩展名。
在创建组件时，用户可以选择自己添加的模板文件，插件会根据用户选择的模板文件生成组件代码。

目前可以替换的模板变量：
- `<ComponentName>`: 组件名称
- `<HostName>`: 宿主类型名称(蛇形命名)
- `<HostType>`: 宿主类型名称(驼峰命名)



## 未来计划

- [完成] 计划在创建组建时可以选择模板，这样户可以添加自己的模板文件，使其更加适用于各种不同的项目。
- [TODO] 支持添加自定义的模板变量，用户可以在模板中添加自己的占位符，插件会根据用户输入的组件名称和宿主类型名称替换模板中的占位符。
